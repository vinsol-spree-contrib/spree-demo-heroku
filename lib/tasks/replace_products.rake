...
namespace :db do
  ...
  task replace_products: :environment do
    ...
    CSV.foreach(Rails.root.join('public', 'products.csv'), headers: true) do |row|
      ...
      if row['Variant']
        variant_0 = Spree::OptionType.find_by_name('Strap Material').id
        variant_1 = Spree::OptionType.find_by_name('Strap Colour').id
        variant_2 = Spree::OptionType.find_by_name('Dial Colour').id
